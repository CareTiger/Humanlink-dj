...
from filebrowser.functions import path_to_url, sort_by_attr, get_path, get_file, get_version_path, get_breadcrumbs, \
	get_filterdate, get_settings_var, handle_file_upload, convert_filename
...
def browse(request):
	...
	return render_to_response(template, context, context_instance=Context(request))
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			try:
				...
				# remove pagination
				redirect_url = reverse("fb_browse") + query_helper(query, "ot=desc,o=date",
...
def _upload_file(request):
	...
	if request.method == 'POST':
		...
		if request.FILES:
			...
			if os.path.isfile(
					smart_str(os.path.join(fb_settings.MEDIA_ROOT, fb_settings.DIRECTORY, folder, filedata.name))):
			...
			# POST UPLOAD SIGNAL
			filebrowser_post_upload.send(sender=request, path=request.POST.get('folder'), file=FileObject(
				smart_str(os.path.join(fb_settings.DIRECTORY, folder, filedata.name))))
			...
			try:
				...
				for version in VERSIONS:
					try:
						...
						logger.error('Error removing image version. %s', exc)
						pass
